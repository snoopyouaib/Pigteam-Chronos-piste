...
#include "lvgl.h"
// #include "demos/lv_demos.h" -- retire, lv_demo_widgets() remplace par
// un simple rectangle plein ecran rouge plus bas (evite de devoir activer
// LV_USE_DEMO_WIDGETS dans lv_conf.h pour ce test minimal)
...
void setup()
{
  Serial.begin(115200);
  delay(1000);
  Serial.println("=== raw_official_test -- .ino officiel Waveshare quasi tel quel, via PlatformIO ===");
  ...
  if (example_lvgl_lock(-1))
  {
    ...
    lv_obj_set_style_bg_color(scr, lv_color_make(0xFF, 0x00, 0x00), 0); // rouge plein ecran
  ...
  Serial.println("Pret -- ecran normalement rouge plein maintenant.");
}
void loop()
{
