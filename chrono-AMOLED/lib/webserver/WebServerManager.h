...
struct WebServerStatusInfo {
  ...
  // n'est actif. usedBytes/totalBytes/usedBytes()/totalBytes() ne font
  // PAS partie de l'interface commune fs::FS (chaque implementation les
  // ajoute a part) -- main.cpp, qui connait le type concret (SD, FFat...),
  // les fournit donc ici plutot que WebServerManager ne les lise via
  // g_logsFs (fs::FS* generique, ne les exposerait pas).
