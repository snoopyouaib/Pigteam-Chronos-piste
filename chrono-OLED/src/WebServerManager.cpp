...
struct LapDetail {
  int number = 0;
  String time;    // tel qu'ecrit dans le CSV, ex "1:03.256"
  String circuit;
};

// Convertit "M:SS.mmm" ou "SS.mmm" en millisecondes -- pour calculer le
// diff et trier/comparer les tours entre eux. Retourne -1 si illisible
// (tour ignore pour le calcul du meilleur, mais reste affiche).
static long lapTimeToMs(const String& t) {
  int colon = t.indexOf(':');
  String secPart = (colon >= 0) ? t.substring(colon + 1) : t;
  long minutes = (colon >= 0) ? t.substring(0, colon).toInt() : 0;
  float seconds = secPart.toFloat();
  if (minutes == 0 && seconds == 0 && secPart != "0" && secPart.length() > 0 && secPart[0] != '0') return -1;
...
static std::vector<LapDetail> loadLapsForSession(const String& targetCompactKey) {
  std::vector<LapDetail> result;
  File f = LittleFS.open(g_sessionLogPath, "r");
  if (!f) return result;

  bool inTargetSession = false;
  while (f.available()) {
    String line = f.readStringUntil('\n');
    line.trim();
    if (line.length() == 0 || line.startsWith("date,")) continue;
    ...
    if (line.startsWith("# session demarree")) {
      String rest = line.substring(line.indexOf("demarree") + 9);
      inTargetSession = (stripSeparators(rest) == targetCompactKey);
      continue;
    }
    if (line.startsWith("# session arretee")) { inTargetSession = false; continue; }
    if (!inTargetSession) continue;
...
static String pageHeader(const char* activeTab) {
  ...
  html += "<a href='/compare'"; if (strcmp(activeTab, "compare") == 0) html += " class='active'"; html += ">Comparer</a>";
...
static void handleHomePage() {
  ...
  //
  // Pour l'instant hasSeparateLogsFs reste toujours false sur cette
  // variante OLED (aucune SD cablee) -- cette barre reste donc invisible
  // en pratique tant que le cablage cote main.cpp n'est pas fait (cf.
  // README_FS.md, section "Checklist pour refaire cette integration").
  //
  // Note historique : le commentaire precedent ici affirmait que la lib
  // SD tierce ne supportait pas l'ESP32-S3 -- en realite, ce n'etait pas
  // une incompatibilite materielle mais un conflit de nom PlatformIO
  // (deux libs homonymes "SD", celle du framework vs celle du registre
  // public) resolu cote TFT via un lib_deps explicite pointant sur la
  // lib du framework (cf. README_FS.md). A refaire ici le jour ou la SD
  // sera cablee sur cette variante.
  {
    WebServerStatusInfo s;
    if (g_getStatus) s = g_getStatus();
  ...
  if (files.empty()) {
    ...
    for (size_t i = 0; i < files.size(); i++) {
      ...
      // ----- Tableau par tour, style RaceChrono : temps + diff colore vs le meilleur -----
      std::vector<LapDetail> laps = loadLapsForSession(compactKey);
      if (!laps.empty()) {
        long bestMs = -1;
        for (const LapDetail& lap : laps) {
          long ms = lapTimeToMs(lap.time);
          if (ms >= 0 && (bestMs < 0 || ms < bestMs)) bestMs = ms;
        }
        html += "<details style='margin-top:10px'><summary style='cursor:pointer;font-size:13px;opacity:0.85'>Detail par tour (" + String(laps.size()) + ")</summary>";
        html += "<table style='width:100%;border-collapse:collapse;margin-top:8px;font-size:13px'>";
        html += "<tr style='opacity:0.7;text-align:left'><th style='padding:4px 8px'>Tour</th><th style='padding:4px 8px'>Temps</th><th style='padding:4px 8px'>Diff</th><th style='padding:4px 8px'>Trace</th></tr>";
        for (const LapDetail& lap : laps) {
          long ms = lapTimeToMs(lap.time);
          bool isBest = (bestMs >= 0 && ms == bestMs);
          String rowStyle, diffText;
          if (ms < 0) {
            rowStyle = "opacity:0.6";
            diffText = "--";
          } else if (isBest) {
            rowStyle = "background:#1d3a5c"; // bleu -- meme code que le tour de reference RaceChrono
            diffText = "0.000";
          } else {
            long diffMs = ms - bestMs;
            rowStyle = "background:#3a1d1d;color:#ff9d9d"; // rouge -- plus lent que le meilleur (pas de vert ici : aucun tour n'est "en cours" sur un historique deja termine)
            diffText = "+" + String(diffMs / 1000.0f, 3);
          }
          html += "<tr style='" + rowStyle + "'>";
          html += "<td style='padding:4px 8px'>" + String(lap.number) + "</td>";
          html += "<td style='padding:4px 8px'><b>" + lap.time + "</b></td>";
          html += "<td style='padding:4px 8px'>" + diffText + "</td>";
          html += "<td style='padding:4px 8px'><a class='file' href='/lap?file=" + shortName + "&lap=" + String(lap.number) + "'>Voir</a></td>";
          html += "</tr>";
        }
        html += "</table>";

        // ----- Evolution des temps au tour sur la session -----
        // Petit graphe SVG genere directement cote serveur (pas de JS, pas
        // de fetch du CSV GPS -- les temps de tour sont deja en memoire ici,
        // contrairement a /compare qui a besoin des points GPS bruts pour
        // les tracer/comparer). Utile pour reperer une derive au fil de la
        // session (pneus qui chauffent, fatigue...). X = position dans la
        // liste des tours VALIDES (pas le numero de tour brut, pour ne pas
        // laisser de trou visuel sur un tour DNF/incomplet), Y = temps,
        // inverse (plus rapide = plus haut sur le graphe -- "progresser" se
        // lit visuellement vers le haut, comme attendu).
        {
          std::vector<long> validMs;
          std::vector<int> validNumbers;
          for (const LapDetail& lap : laps) {
            long ms = lapTimeToMs(lap.time);
            if (ms >= 0) { validMs.push_back(ms); validNumbers.push_back(lap.number); }
          }
          if (validMs.size() >= 2) {
            long minMs = validMs[0], maxMs = validMs[0];
            for (long ms : validMs) { if (ms < minMs) minMs = ms; if (ms > maxMs) maxMs = ms; }
            if (minMs == maxMs) maxMs = minMs + 1; // evite une division par zero si tous les tours sont identiques

            // Marges agrandies par rapport a une simple courbe -- il faut la
            // place en haut/bas pour la legende +vite/-vite (centree, sous
            // et au-dessus du graphe) et pour le numero de tour sous chaque
            // point, sans chevaucher la courbe.
            const int W = 600, H = 180, padTop = 30, padBottom = 46, padSide = 24;
            html += "<div style='margin-top:10px'><div class='meta' style='margin-bottom:4px'>Evolution des temps au tour</div>";
            html += "<svg viewBox='0 0 " + String(W) + " " + String(H) + "' style='width:100%;max-width:600px;background:#0c0f13;border-radius:8px;border:1px solid #2a323c'>";

            auto xOf = [&](size_t i) { return padSide + (validMs.size() > 1 ? (float)i / (validMs.size() - 1) : 0.5f) * (W - 2 * padSide); };
            auto yOf = [&](long ms) { return padTop + (float)(ms - minMs) / (float)(maxMs - minMs) * (H - padTop - padBottom); };

            // Mediane -- ligne de reperage horizontale, plus robuste qu'une
            // moyenne si un tour est anormalement lent/rapide (sortie large,
            // trafic...) qui fausserait davantage une simple moyenne.
            std::vector<long> sortedMs = validMs;
            std::sort(sortedMs.begin(), sortedMs.end());
            size_t mid = sortedMs.size() / 2;
            long medianMs = (sortedMs.size() % 2 == 1) ? sortedMs[mid] : (sortedMs[mid - 1] + sortedMs[mid]) / 2;

            html += "<line x1='" + String(padSide) + "' y1='" + String(yOf(medianMs), 1) + "' x2='" + String(W - padSide) +
                    "' y2='" + String(yOf(medianMs), 1) + "' stroke='#555' stroke-width='1' stroke-dasharray='4,4'/>";

            // Legende fixe, centree en haut/bas du graphe -- explique le sens
            // de l'axe (inverse : plus haut = plus rapide) d'un coup d'oeil,
            // plutot que de repeter un tag a chaque point.
            html += "<text x='" + String(W / 2) + "' y='16' text-anchor='middle' font-size='11' fill='#7ee787'>+vite</text>";
            html += "<text x='" + String(W / 2) + "' y='" + String(H - 8) + "' text-anchor='middle' font-size='11' fill='#ff8585'>-vite</text>";

            String points = "";
            for (size_t i = 0; i < validMs.size(); i++) points += String(xOf(i), 1) + "," + String(yOf(validMs[i]), 1) + " ";
            html += "<polyline points='" + points + "' fill='none' stroke='#3a7bd5' stroke-width='2'/>";

            for (size_t i = 0; i < validMs.size(); i++) {
              bool isBest = (validMs[i] == minMs);
              float x = xOf(i), y = yOf(validMs[i]);
              html += "<circle cx='" + String(x, 1) + "' cy='" + String(y, 1) + "' r='" + String(isBest ? 5 : 3) +
                      "' fill='" + String(isBest ? "#7ee787" : "#3a7bd5") + "'/>";

              // Numero de tour sous le point (toujours affiche).
              html += "<text x='" + String(x, 1) + "' y='" + String(y + 16, 1) +
                      "' text-anchor='middle' font-size='10' fill='#9aa4b2'>T" + String(validNumbers[i]) + "</text>";
            }
...
// ===================== Page "/compare" -- superposition de plusieurs tours d'UNE session =====================
//
// Remplace l'ancienne version (deux sessions completes superposees) --
// peu utile en pratique, un chrono complet contient surtout des lignes
// droites/paddock qui n'apportent rien a la comparaison. Ici : on
// choisit UNE session, puis on coche jusqu'a 3 de ses tours (au-dela le
// trace devient illisible sur un petit ecran de telephone), et seuls
// ces tours-la sont superposes -- meme decoupage par tour que /lap (cf.
// colonne "laps" du log detaille), mais couleur unie par tour au lieu du
// degrade par vitesse (qui n'a de sens que pour un tour affiche seul).
//
// Pas de fond de carte reel (OpenStreetMap etc.) -- ca demanderait une
// connexion Internet que le telephone n'a plus une fois connecte au
// point d'acces de l'ESP. La trajectoire seule suffit largement a
// reconnaitre un circuit (la forme parle d'elle-meme), donc tout reste
// utilisable au bord de la piste, sans reseau. Le calcul de projection
// (lat/lon -> plan local en metres) et le dessin SVG se font entierement
// cote navigateur -- l'ESP se contente de servir le CSV brut via la
// route /download deja existante.
...
// JS commun a /compare et /lap -- parsing du CSV, distance haversine,
// projection lat/lon -> plan local en metres, et mise a l'echelle vers
// des coordonnees ecran. Etait duplique a l'identique dans les deux
// pages (constate lors de leur ecriture successive) ; factorise ici et
// injecte en tete du <script> des deux pages plutot que copie-colle --
// une seule version a maintenir si la logique de parsing/projection
// change un jour. Les champs "laps"/"current_lap_ms" ne sont utiles qu'a
// /lap (decoupage par tour), mais les inclure ici ne coute rien a
// /compare (simplement ignores).
static String sharedTraceJs() {
  return R"JS(
function parseCsv(text) {
  const lines = text.split(/\r?\n/).map(l => l.trim()).filter(l => l.length > 0);
  if (lines.length < 2) return [];
  const header = lines[0].split(',');
  const idx = { lat: header.indexOf('lat'), lng: header.indexOf('lng'), speed: header.indexOf('speed_kmh'), fix: header.indexOf('fix'), laps: header.indexOf('laps'), lapMs: header.indexOf('current_lap_ms'), localTime: header.indexOf('local_time') };
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const fix = idx.fix >= 0 ? parseInt(cols[idx.fix]) || 0 : 3;
    if (fix < 2) continue; // pas de fix fiable -- meme filtre que l'outil de bureau
    const laps = idx.laps >= 0 ? parseInt(cols[idx.laps]) || 0 : -1;
    const lapMs = idx.lapMs >= 0 ? parseInt(cols[idx.lapMs]) || 0 : 0;
    const localTime = idx.localTime >= 0 ? cols[idx.localTime] : '';
    rows.push({ lat, lng, speed: idx.speed >= 0 ? parseFloat(cols[idx.speed]) || 0 : 0, laps, lapMs, localTime });
  }
  return rows;
}
...
// Projection plane locale (equirectangulaire) centree sur un point de
// reference (premier point du tracé/tour) -- suffisant a l'echelle d'un
// circuit (quelques centaines de metres a 1-2km), l'erreur de courbure
// terrestre y est negligeable.
function project(rows, refLat, refLng) {
  const mPerDegLat = 111320;
  const mPerDegLng = 111320 * Math.cos(refLat * Math.PI / 180);
  return rows.map(r => ({ x: (r.lng - refLng) * mPerDegLng, y: (r.lat - refLat) * mPerDegLat, speed: r.speed }));
}

// Mise a l'echelle metres -> pixels ecran (conserve les proportions,
// centre dans le cadre, inverse Y -- latitude nord = vers le haut).
function toScreen(points, minX, maxX, minY, maxY, W, H, pad) {
  const spanX = Math.max(maxX - minX, 1), spanY = Math.max(maxY - minY, 1);
  const scale = Math.min((W - 2*pad) / spanX, (H - 2*pad) / spanY);
  const offX = pad + ((W - 2*pad) - spanX * scale) / 2;
  const offY = pad + ((H - 2*pad) - spanY * scale) / 2;
  return points.map(p => ({ sx: offX + (p.x - minX) * scale, sy: H - (offY + (p.y - minY) * scale) }));
}
)JS";
}

static void handleComparePage() {
  String file = httpServer.arg("file");

  std::vector<std::pair<String, uint32_t>> files;
  forEachGpsLogFile([&](const String& name, uint32_t size) { files.push_back({name.substring(1), size}); });
  std::sort(files.begin(), files.end(), [](const std::pair<String,uint32_t>& x, const std::pair<String,uint32_t>& y) {
    return x.first > y.first;
  });

  String html = pageHeader("compare");
  html += "<h2>Comparer des tours</h2>";

  if (files.empty()) {
    html += "<p><i>Aucune session enregistree pour l'instant.</i></p>";
    html += PAGE_FOOTER;
    httpServer.send(200, "text/html", html);
    return;
  }

  // ----- Etape 1 : choix de la session -- auto-soumis au changement -----
  html += "<form method='GET' action='/compare' style='display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin:12px 0'>";
  html += "<select name='file' onchange='this.form.submit()' style='background:#1d242c;color:#e8eaed;border:1px solid #2a323c;border-radius:8px;padding:8px 10px;font-size:13px'>";
  html += "<option value=''>-- choisir une session --</option>";
  for (size_t i = 0; i < files.size(); i++) {
    const String& fn = files[i].first;
    html += "<option value='" + fn + "'";
    if (fn == file) html += " selected";
    html += ">" + compareFileLabel(fn) + "</option>";
  }
  html += "</select>";
  html += "</form>";

  bool fileOk = file.length() > 0 && isAllowedGpsLogFile("/" + file);
  if (!fileOk) {
    html += "<p><i>Choisis une session ci-dessus pour voir la liste de ses tours.</i></p>";
    html += PAGE_FOOTER;
    httpServer.send(200, "text/html", html);
    return;
  }

  // ----- Etape 2 : tableau des tours de cette session, avec case a
  // cocher par ligne pour la comparaison (max 3) -----
  // Reprend les memes tours/temps que le tableau "Detail par tour" de la
  // page Sessions (meme fonction loadLapsForSession(), meme recalcul du
  // meilleur temps reel) -- coherence garantie entre les deux pages.
  String compactKey = compactKeyFromLogFilename("/" + file);
  std::vector<LapDetail> sessionLaps = loadLapsForSession(compactKey);

  long bestMs = -1;
  for (const LapDetail& lap : sessionLaps) {
    long ms = lapTimeToMs(lap.time);
    if (ms >= 0 && (bestMs < 0 || ms < bestMs)) bestMs = ms;
  }
  ...
  int bestLapNumber = -1;
  for (const LapDetail& lap : sessionLaps) {
    if (bestMs >= 0 && lapTimeToMs(lap.time) == bestMs) { bestLapNumber = lap.number; break; }
  }

  String circuitName = "?";
  for (const LapDetail& lap : sessionLaps) { if (lap.circuit.length() > 0) { circuitName = lap.circuit; break; } }

  // ----- Resume de session, façon RaceChrono -----
  // Distance totale = calculee cote JS (script juste apres les cases a
  // cocher plus bas) a partir du CSV complet, pas seulement des tours
  // coches -- necessite de parcourir tout le fichier, pas de raison de le
  // faire cote firmware alors que le navigateur le fait deja pour le
  // trace. Tour optimal (meilleur temps theorique = somme des meilleurs
  // secteurs) laisse en placeholder pour l'instant -- ce chrono n'a pas
  // de notion de secteurs/splits aujourd'hui, methode de calcul a definir.
  html += "<div class='card' style='display:block'>";
  html += "<div style='display:flex;gap:24px;flex-wrap:wrap'>";
  html += "<div><div class='meta'>Circuit</div><b>" + circuitName + "</b></div>";
  html += "<div><div class='meta'>Tours</div><b>" + String(sessionLaps.size()) + "</b></div>";
  html += "<div><div class='meta'>Meilleur tour</div><b>" + msToLapTime(bestMs) + "</b></div>";
  html += "<div><div class='meta'>Tour optimal</div><b id='optimalLap'>--:--.---</b></div>";
  html += "<div><div class='meta'>Distance totale</div><b id='totalDistance'>...</b></div>";
  html += "</div></div>";

  // Selection courante -- parametres "lap" repetes (cases a cocher de
  // meme nom), l'URL /compare?file=...&lap=1&lap=3 reste partageable/
  // rechargeable telle quelle, comme avant avec a=/b=.
  std::vector<int> selectedLaps;
  for (int i = 0; i < httpServer.args(); i++) {
    if (httpServer.argName(i) == "lap") {
      int n = httpServer.arg(i).toInt();
      if (n > 0) selectedLaps.push_back(n);
    }
  }
  if (selectedLaps.size() > 3) selectedLaps.resize(3); // securite si l'URL est modifiee a la main
  auto isSelected = [&](int n) { for (int s : selectedLaps) if (s == n) return true; return false; };

  if (sessionLaps.empty()) {
    html += "<p><i>Aucun tour valide dans cette session.</i></p>";
    html += PAGE_FOOTER;
    httpServer.send(200, "text/html", html);
    return;
  }

  html += "<form method='GET' action='/compare' style='margin:12px 0'>";
  html += "<input type='hidden' name='file' value='" + file + "'>";
  html += "<div style='overflow-x:auto'>";
  html += "<table id='lapsTable' style='width:100%;border-collapse:collapse;margin-bottom:10px;font-size:13px;white-space:nowrap'>";
  html += "<tr style='opacity:0.7;text-align:left'>";
  html += "<th style='padding:4px 6px'></th><th style='padding:4px 6px'>Tour</th><th style='padding:4px 6px'>Temps</th>";
  html += "<th style='padding:4px 6px'>Diff</th><th style='padding:4px 6px'>Depart</th><th style='padding:4px 6px'>Distance</th>";
  html += "<th style='padding:4px 6px'>V.max</th><th style='padding:4px 6px'>V.min</th><th style='padding:4px 6px'>V.moy</th>";
  html += "</tr>";
  for (const LapDetail& lap : sessionLaps) {
    long ms = lapTimeToMs(lap.time);
    bool isBest = (bestMs >= 0 && ms == bestMs);
    ...
    else if (ms >= 0) diffText = "+" + String((ms - bestMs) / 1000.0f, 3);
    String n = String(lap.number);
    html += "<tr style='" + String(isBest ? "background:#1d3a5c" : "") + "'>";
    html += "<td style='padding:4px 6px'><input type='checkbox' name='lap' value='" + n + "'";
    if (isSelected(lap.number)) html += " checked";
    html += "></td>";
    html += "<td style='padding:4px 6px'>" + n + "</td>";
    html += "<td style='padding:4px 6px'><b>" + lap.time + "</b></td>";
    html += "<td style='padding:4px 6px'>" + diffText + "</td>";
    html += "<td style='padding:4px 6px' id='start-" + n + "'>...</td>";
    html += "<td style='padding:4px 6px' id='dist-" + n + "'>...</td>";
    html += "<td style='padding:4px 6px' id='vmax-" + n + "'>...</td>";
    html += "<td style='padding:4px 6px' id='vmin-" + n + "'>...</td>";
    html += "<td style='padding:4px 6px' id='vavg-" + n + "'>...</td>";
    html += "</tr>";
  }
  html += "</table></div>";
  html += "<button type='submit' style='background:#3a7bd5'>Comparer</button>";
  html += "<span style='opacity:0.6;font-size:12px;margin-left:8px'>3 tours maximum</span>";
  html += "</form>";

  // Limite cote client a 3 cases cochees simultanement -- desactive les
  // cases restantes plutot que de les laisser cochables sans effet
  // (plus clair pour l'utilisateur que la limite silencieuse cote
  // serveur qui suit juste au cas ou).
  html += "<script>";
  html += "(function(){";
  html += "var boxes = document.querySelectorAll('#lapsTable input[type=checkbox]');";
  html += "function refresh(){ var n = 0; boxes.forEach(function(b){ if (b.checked) n++; }); boxes.forEach(function(b){ if (!b.checked) b.disabled = (n >= 3); }); }";
  html += "boxes.forEach(function(b){ b.addEventListener('change', refresh); }); refresh();";
  html += "})();";
  html += "</script>";

  // ----- Script : distance totale de la session + remplissage du tableau
  // (Depart/Distance/V.max/V.min/V.moy pour CHAQUE tour, pas seulement
  // ceux coches) -- tourne toujours des qu'une session est choisie,
  // independamment de la comparaison plus bas. extractLap()/statsOf()
  // sont declarees ici (portee globale, scripts classiques) et reutilisees
  // telles quelles par le script de comparaison plus bas -- pas de raison
  // de les redeclarer une 2e fois dans la meme page.
  html += "<script>";
  html += sharedTraceJs();
  html += "const logFileSummary='" + file + "';";
  html += "const bestLapNumber=" + String(bestLapNumber) + ";";
  html += "const allLapNumbers=[";
  for (size_t i = 0; i < sessionLaps.size(); i++) { if (i) html += ","; html += String(sessionLaps[i].number); }
  html += "];";
  html += R"JS(
// Reprend le decoupage de /lap : filtre par "laps" (nb de tours DEJA
// valides au moment de la ligne), puis tronque au dernier point ayant
// current_lap_ms minimal (evite d'inclure l'attente immobile au paddock
// avant le declenchement du 1er tour, qui partage le meme compteur).
function extractLap(rows, lapNumber) {
  const lapRows = rows.filter(r => r.laps === lapNumber - 1);
  if (lapRows.length === 0) return [];
  const minLapMs = Math.min(...lapRows.map(r => r.lapMs));
  let startIdx = 0;
  for (let i = 0; i < lapRows.length; i++) if (lapRows[i].lapMs === minLapMs) startIdx = i;
  return lapRows.slice(startIdx);
}

function statsOf(rows) {
  if (rows.length === 0) return { points: 0, distKm: 0, maxSpeed: 0, minSpeed: 0, avgSpeed: 0, durationS: 0, startTime: '' };
  let dist = 0, maxSpeed = -Infinity, minSpeed = Infinity, sumSpeed = 0;
  for (const r of rows) {
    maxSpeed = Math.max(maxSpeed, r.speed);
    minSpeed = Math.min(minSpeed, r.speed);
    sumSpeed += r.speed;
  }
  for (let i = 1; i < rows.length; i++) dist += haversineM(rows[i-1].lat, rows[i-1].lng, rows[i].lat, rows[i].lng);
  return { points: rows.length, distKm: dist / 1000, maxSpeed, minSpeed, avgSpeed: sumSpeed / rows.length, durationS: rows.length / 10, startTime: rows[0].localTime };
}

(async function() {
  const text = await fetch('/download?file=' + encodeURIComponent(logFileSummary)).then(r => r.text());
  ...
  // Un seul passage prealable pour avoir toutes les stats sous la main
  // (necessaire pour comparer chaque tour aux valeurs du meilleur tour,
  // pas seulement a lui-meme).
  const statsByLap = {};
  for (const n of allLapNumbers) statsByLap[n] = statsOf(extractLap(rows, n));
  const bestStats = statsByLap[bestLapNumber];

  // Vert si meilleur (plus rapide) que le meilleur tour sur cette colonne
  // precise, rouge si moins bon -- inspire de RaceChrono. Uniquement sur
  // V.max/V.min/V.moy : plus haut = toujours mieux pour ces 3-la. Pas
  // applique a Distance (une distance plus courte n'est pas forcement une
  // meilleure trajectoire, juste une ligne differente) ni au tour meilleur
  // lui-meme (comparaison a soi-meme, non pertinente).
  const colorFor = (value, bestValue) => {
    if (bestValue === undefined || value === bestValue) return '';
    return value > bestValue ? 'color:#7ee787' : 'color:#ff8585';
  };
  const set = (id, text, extraStyle) => {
  ...
  for (const n of allLapNumbers) {
    const s = statsByLap[n];
    if (s.points === 0) {
      set('start-' + n, '--'); set('dist-' + n, '--'); set('vmax-' + n, '--'); set('vmin-' + n, '--'); set('vavg-' + n, '--');
      continue;
    }
    const isBestRow = (n === bestLapNumber);
    set('start-' + n, s.startTime || '--');
    set('dist-' + n, s.distKm.toFixed(2) + ' km');
    set('vmax-' + n, fmtSpeed(s.maxSpeed), isBestRow ? '' : colorFor(s.maxSpeed, bestStats && bestStats.maxSpeed));
    set('vmin-' + n, fmtSpeed(s.minSpeed), isBestRow ? '' : colorFor(s.minSpeed, bestStats && bestStats.minSpeed));
    set('vavg-' + n, fmtSpeed(s.avgSpeed), isBestRow ? '' : colorFor(s.avgSpeed, bestStats && bestStats.avgSpeed));
  }
})();
)JS";
  html += "</script>";

  if (selectedLaps.empty()) {
    html += "<p><i>Coche un ou plusieurs tours ci-dessus puis clique \"Comparer\".</i></p>";
    html += PAGE_FOOTER;
    httpServer.send(200, "text/html", html);
    return;
  }

  // ----- Rendu du trace -- un seul CSV recupere, filtre par tour cote JS -----
  html += "<div id='stats' style='display:flex;gap:16px;flex-wrap:wrap;margin:10px 0;font-size:13px'></div>";
  html += "<svg id='chart' viewBox='0 0 800 600' style='width:100%;max-width:800px;background:#0c0f13;border-radius:10px;border:1px solid #2a323c'></svg>";

  // Graphe d'ecart de temps -- n'a de sens qu'a partir de 2 tours (besoin
  // d'une reference a laquelle comparer les autres), donc absent du DOM
  // si un seul tour est coche plutot que present mais vide.
  if (selectedLaps.size() >= 2) {
    html += "<h3>Ecart de temps (vs le tour le plus rapide coche)</h3>";
    html += "<svg id='gapChart' viewBox='0 0 800 300' style='width:100%;max-width:800px;background:#0c0f13;border-radius:10px;border:1px solid #2a323c'></svg>";
  }

  html += "<script>";
  html += "const logFile='" + file + "';";
  html += "const targetLaps=[";
  for (size_t i = 0; i < selectedLaps.size(); i++) { if (i) html += ","; html += String(selectedLaps[i]); }
  html += "];";
  html += R"JS(
// extractLap()/statsOf()/parseCsv()/project()/toScreen()/haversineM() sont
// deja globales (declarees par le script juste au-dessus, qui tourne
// toujours en premier -- meme page, meme ordre d'execution des <script>).
const COLORS = ['#d69e2e', '#3ecfcf', '#e05fa0'];

async function run() {
  const text = await fetch('/download?file=' + encodeURIComponent(logFile)).then(r => r.text());
  const allRows = parseCsv(text);
  const perLap = targetLaps.map(n => extractLap(allRows, n));

  if (perLap.every(r => r.length === 0)) {
    document.getElementById('stats').innerHTML = '<i>Aucun tour exploitable dans ce log (log trop ancien, sans colonne "laps" ?).</i>';
    return;
  }

  const refRows = perLap.find(r => r.length > 0);
  const refLat = refRows[0].lat, refLng = refRows[0].lng;
  const projAll = perLap.map(rows => project(rows, refLat, refLng));
  const flat = projAll.flat();
  const minX = Math.min(...flat.map(p => p.x)), maxX = Math.max(...flat.map(p => p.x));
  const minY = Math.min(...flat.map(p => p.y)), maxY = Math.max(...flat.map(p => p.y));

  const svg = document.getElementById('chart');
  const W = 800, H = 600, pad = 30;
  let svgContent = '';
  projAll.forEach((proj, i) => {
    if (proj.length > 1) {
      const pts = toScreen(proj, minX, maxX, minY, maxY, W, H, pad).map(p => p.sx.toFixed(1) + ',' + p.sy.toFixed(1)).join(' ');
      svgContent += `<polyline points="${pts}" fill="none" stroke="${COLORS[i % COLORS.length]}" stroke-width="3" opacity="0.9"/>`;
    }
  });
  svg.innerHTML = svgContent;

  const fmtDur = s => Math.floor(s/60) + 'min ' + Math.round(s%60) + 's';
  document.getElementById('stats').innerHTML = targetLaps.map((n, i) => {
    const s = statsOf(perLap[i]);
    if (s.points === 0) return `<div><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${COLORS[i % COLORS.length]};margin-right:6px"></span><b>Tour ${n}</b><br><i>introuvable dans le log</i></div>`;
    return `<div><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${COLORS[i % COLORS.length]};margin-right:6px"></span><b>Tour ${n}</b><br>${s.points} pts -- ${s.distKm.toFixed(2)}km -- vmax ${s.maxSpeed.toFixed(0)}km/h -- ${fmtDur(s.durationS)}</div>`;
  }).join('');

  // ----- Graphe d'ecart de temps (gap plot) -----
  // Pour chaque tour, construit une courbe distance -> temps ecoule (a
  // partir de current_lap_ms, deja mesure par le firmware -- plus fiable
  // qu'un recalcul base sur l'index/frequence d'echantillonnage). Le tour
  // de reference (ligne zero) est celui qui boucle le plus vite parmi
  // ceux coches -- l'ecart des autres est trace par rapport a lui :
  // positif = plus lent que la reference a cet endroit du circuit,
  // negatif = plus rapide. Comparaison bornee a la distance du tour
  // coche le plus court (au-dela, plus de point de reference valable).
  const gapSvg = document.getElementById('gapChart');
  if (gapSvg) {
    function buildDistTime(rows) {
      const baseline = rows[0].lapMs;
      let dist = 0;
      const pts = [{ d: 0, t: 0 }];
      for (let i = 1; i < rows.length; i++) {
        dist += haversineM(rows[i-1].lat, rows[i-1].lng, rows[i].lat, rows[i].lng);
        pts.push({ d: dist, t: (rows[i].lapMs - baseline) / 1000 });
      }
      return pts;
    }

    // Interpolation lineaire de t en fonction de d (recherche dichotomique
    // -- series pouvant compter plusieurs milliers de points a 10Hz).
    function timeAtDistance(series, targetD) {
      if (targetD <= series[0].d) return series[0].t;
      const last = series[series.length - 1];
      if (targetD >= last.d) return last.t;
      ...
      const frac = (targetD - a.d) / (b.d - a.d || 1);
      return a.t + frac * (b.t - a.t);
    }

    const series = perLap.map(rows => rows.length > 1 ? buildDistTime(rows) : null);
    const validIdx = series.map((s, i) => s ? i : -1).filter(i => i >= 0);

    if (validIdx.length < 2) {
      gapSvg.outerHTML = "<p><i>Pas assez de tours exploitables pour tracer l'ecart.</i></p>";
    } else {
      let refIdx = validIdx[0];
      for (const i of validIdx) {
        if (series[i][series[i].length - 1].t < series[refIdx][series[refIdx].length - 1].t) refIdx = i;
      }
      const refSeries = series[refIdx];
      const maxDist = Math.min(...validIdx.map(i => series[i][series[i].length - 1].d));

      const GW = 800, GH = 300, gpad = 36, SAMPLES = 200;
      const deltasByLap = {};
      let maxAbsDelta = 0.1; // plancher -- evite un axe Y ecrase si tous les tours sont quasi identiques
      for (const i of validIdx) {
        if (i === refIdx) continue;
        const pts = [];
        for (let s = 0; s <= SAMPLES; s++) {
          const d = (maxDist * s) / SAMPLES;
          const delta = timeAtDistance(series[i], d) - timeAtDistance(refSeries, d);
          pts.push({ d, delta });
      ...
      const xForD = d => gpad + (d / maxDist) * (GW - 2 * gpad);
      const yForDelta = delta => GH / 2 - (delta / maxAbsDelta) * (GH / 2 - gpad);

      let gapContent = '';
      gapContent += `<line x1="${gpad}" y1="${GH/2}" x2="${GW-gpad}" y2="${GH/2}" stroke="#444" stroke-width="1" stroke-dasharray="4,4"/>`;
      gapContent += `<text x="${gpad}" y="${GH/2 - 8}" font-size="11" fill="#888">Tour ${targetLaps[refIdx]} = reference</text>`;
      gapContent += `<text x="${gpad}" y="16" font-size="11" fill="#888">+${maxAbsDelta.toFixed(1)}s (plus lent)</text>`;
      gapContent += `<text x="${gpad}" y="${GH - 6}" font-size="11" fill="#888">-${maxAbsDelta.toFixed(1)}s (plus rapide)</text>`;

      Object.keys(deltasByLap).forEach(key => {
        const i = Number(key);
        const pts = deltasByLap[i];
        const path = pts.map(p => xForD(p.d).toFixed(1) + ',' + yForDelta(p.delta).toFixed(1)).join(' ');
        gapContent += `<polyline points="${path}" fill="none" stroke="${COLORS[i % COLORS.length]}" stroke-width="2.5"/>`;
      });

      gapSvg.innerHTML = gapContent;
    }
  }
}
run();
...
static void handleLapTracePage() {
  ...
  // ----- Navigation precedent/suivant entre les tours de la MEME session -----
  // Base sur la liste reelle des tours du carnet (pas simplement N-1/N+1)
  // -- au cas ou un tour manquerait (log partiel, tour non valide...),
  // les fleches restent coherentes avec ce qui existe vraiment.
  String compactKey = compactKeyFromLogFilename("/" + file);
  std::vector<LapDetail> sessionLaps = loadLapsForSession(compactKey);
  int prevLap = -1, nextLap = -1;
  for (size_t i = 0; i < sessionLaps.size(); i++) {
    if (sessionLaps[i].number == lapNumber) {
      if (i > 0) prevLap = sessionLaps[i - 1].number;
      if (i + 1 < sessionLaps.size()) nextLap = sessionLaps[i + 1].number;
      break;
    }
  }

  String html = pageHeader("home");
  html += "<h2>Tour " + String(lapNumber) + "</h2>";
  ...
  if (prevLap > 0) {
    html += "<a class='file' href='/lap?file=" + file + "&lap=" + String(prevLap) + "'>&larr; Tour " + String(prevLap) + "</a>";
  } else {
    html += "<span style='opacity:0.3'>&larr; Tour precedent</span>";
  }
  if (nextLap > 0) {
    html += "<a class='file' href='/lap?file=" + file + "&lap=" + String(nextLap) + "'>Tour " + String(nextLap) + " &rarr;</a>";
  } else {
    html += "<span style='opacity:0.3'>Tour suivant &rarr;</span>";
  }
  html += "</div>";
  ...
  html += "<script>";
  html += sharedTraceJs();
  html += "const logFile='" + file + "', targetLap=" + String(lapNumber) + ";";
  html += R"JS(
// Degrade rouge (lent) -> jaune -> vert (rapide), teinte HSL 0deg->120deg --
// passe naturellement par l'orange/jaune au milieu, meme esprit visuel que
// RaceChrono sans avoir a coder une palette a la main.
function speedColor(frac) {
  const hue = Math.max(0, Math.min(1, frac)) * 120;
  return `hsl(${hue.toFixed(0)}, 85%, 50%)`;
}

// Moyenne glissante -- attenue le bruit GPS avant de chercher les
// extrema locaux (sinon chaque micro-oscillation de vitesse devient une
// etiquette).
...
// Detecte les extrema locaux (freinages = min, relances = max) sur une
// serie lissee, avec une separation minimale (en points) et une
// prominence minimale (en km/h) pour ne garder que les points
// significatifs -- pas chaque micro-vague.
function findExtrema(values, minSeparation, minProminence) {
  const candidates = [];
  for (let i = 1; i < values.length - 1; i++) {
    if (values[i] >= values[i-1] && values[i] >= values[i+1] && values[i] > values[i-1]) candidates.push({ index: i, type: 'max', value: values[i] });
    if (values[i] <= values[i-1] && values[i] <= values[i+1] && values[i] < values[i-1]) candidates.push({ index: i, type: 'min', value: values[i] });
  }
  const kept = [];
  for (const c of candidates) {
    if (kept.length === 0) { kept.push(c); continue; }
    const prev = kept[kept.length - 1];
    if (c.index - prev.index < minSeparation) {
      if (c.type === prev.type) {
        if ((c.type === 'max' && c.value > prev.value) || (c.type === 'min' && c.value < prev.value)) kept[kept.length - 1] = c;
      }
      continue; // types opposes trop rapproches -- on ignore le second, garde le premier
    }
    kept.push(c);
  }
  const final = [];
  for (let i = 0; i < kept.length; i++) {
    const c = kept[i];
    const prevVal = i > 0 ? kept[i-1].value : c.value;
    const nextVal = i < kept.length - 1 ? kept[i+1].value : c.value;
    const prom = Math.max(Math.abs(c.value - prevVal), Math.abs(c.value - nextVal));
    if (prom >= minProminence || (i === 0 && kept.length === 1)) final.push(c);
  }
  return final;
}
...
async function run() {
  const text = await fetch('/download?file=' + encodeURIComponent(logFile)).then(r => r.text());
  const allRows = parseCsv(text);
  let rows = allRows.filter(r => r.laps === targetLap - 1);

  // Sur le 1er tour d'une session, le vehicule a pu rester immobile au
  // paddock avant meme de franchir la ligne une premiere fois -- ces
  // points partagent le meme compteur "laps" (0) que le vrai tour
  // chronometre, mais current_lap_ms y reste bloque a 0 tant que le
  // chrono n'a pas reellement demarre. Sur les tours suivants, ce
  // compteur repart de 0 exactement au franchissement de ligne, donc ce
  // filtre ne change rien pour eux (bloc deja bien borne). On ne garde
  // qu'a partir du DERNIER point ayant la valeur minimale du bloc.
  if (rows.length > 0) {
    const minLapMs = Math.min(...rows.map(r => r.lapMs));
    let startIdx = 0;
    for (let i = 0; i < rows.length; i++) if (rows[i].lapMs === minLapMs) startIdx = i;
    rows = rows.slice(startIdx);
  }

  if (rows.length < 3) {
    document.getElementById('stats').innerHTML = '<i>Tour introuvable dans ce log (log trop ancien, sans colonne "laps" ?).</i>';
    return;
  }

  const refLat = rows[0].lat, refLng = rows[0].lng;
  const proj = project(rows, refLat, refLng);
  ...
  const screen = toScreen(proj, minX, maxX, minY, maxY, W, H, pad);
  ...
  let svgContent = '';
  // ----- Trace, un segment colore par la vitesse moyenne du segment -----
  for (let i = 1; i < screen.length; i++) {
    const avgSpeed = (speeds[i-1] + speeds[i]) / 2;
    const frac = (avgSpeed - minSpeed) / speedSpan;
    svgContent += `<line x1="${screen[i-1].sx.toFixed(1)}" y1="${screen[i-1].sy.toFixed(1)}" x2="${screen[i].sx.toFixed(1)}" y2="${screen[i].sy.toFixed(1)}" stroke="${speedColor(frac)}" stroke-width="5" stroke-linecap="round"/>`;
  }

  // ----- Etiquettes de vitesse aux extrema locaux (freinages/relances) -----
  const smoothed = smooth(speeds, 5);
  const minSeparation = Math.max(8, Math.round(rows.length / 20)); // ~qq secondes a 10Hz, adapte a la longueur du tour
  const extrema = findExtrema(smoothed, minSeparation, 8);

  const cx = screen.reduce((s, p) => s + p.sx, 0) / screen.length;
  const cy = screen.reduce((s, p) => s + p.sy, 0) / screen.length;

  for (const e of extrema) {
    const pt = screen[e.index];
    let dx = pt.sx - cx, dy = pt.sy - cy;
    const len = Math.hypot(dx, dy) || 1;
    dx /= len; dy /= len;
    const labelX = pt.sx + dx * 26, labelY = pt.sy + dy * 26;
    const bg = e.type === 'max' ? '#2f7d32' : '#7a1f1f';
    const label = Math.round(e.value).toString();
    const boxW = 15 + label.length * 9, boxH = 22;
    svgContent += `<line x1="${pt.sx.toFixed(1)}" y1="${pt.sy.toFixed(1)}" x2="${labelX.toFixed(1)}" y2="${labelY.toFixed(1)}" stroke="#888" stroke-width="1"/>`;
    svgContent += `<circle cx="${pt.sx.toFixed(1)}" cy="${pt.sy.toFixed(1)}" r="3" fill="#e8eaed"/>`;
    svgContent += `<rect x="${(labelX - boxW/2).toFixed(1)}" y="${(labelY - boxH/2).toFixed(1)}" width="${boxW}" height="${boxH}" rx="5" fill="${bg}"/>`;
    svgContent += `<text x="${labelX.toFixed(1)}" y="${(labelY + 5).toFixed(1)}" text-anchor="middle" font-size="13" font-weight="700" fill="#fff">${label}</text>`;
  }

  // ----- Marqueur Start/Finish (premier point du tour) -----
  const sf = screen[0];
  svgContent += `<circle cx="${sf.sx.toFixed(1)}" cy="${sf.sy.toFixed(1)}" r="6" fill="#3ecfcf" stroke="#0c0f13" stroke-width="2"/>`;
  svgContent += `<text x="${sf.sx.toFixed(1)}" y="${(sf.sy - 12).toFixed(1)}" text-anchor="middle" font-size="12" fill="#3ecfcf">Start/Finish</text>`;

  document.getElementById('chart').innerHTML = svgContent;

  let dist = 0;
  for (let i = 1; i < rows.length; i++) dist += haversineM(rows[i-1].lat, rows[i-1].lng, rows[i].lat, rows[i].lng);
  const durationS = rows.length / 10;
  document.getElementById('stats').innerHTML = `
    <div><b>Distance</b><br>${(dist/1000).toFixed(2)} km</div>
    <div><b>Duree</b><br>${durationS.toFixed(1)} s</div>
    <div><b>Vitesse max</b><br>${maxSpeed.toFixed(0)} km/h</div>
    <div><b>Vitesse min</b><br>${minSpeed.toFixed(0)} km/h</div>
  `;
}
run();
)JS";
...
static const char* SESSIONS_CSV_HEADER = "date,local_time,lap_number,lap_time,best_lap_time,circuit";
...
void WebServerManager::startDownloadMode() {
  ...
  httpServer.on("/compare", HTTP_GET, handleComparePage);
